          ...
          children: [
            ...
            new Paragraph({
              children: [new TextRun({
                text: '⚠ Document d\'aide — vérifiez et reportez les informations sur le formulaire exigé par l\'acheteur.',
                font: 'Arial', size: 18, color: 'D97706', bold: true,
              })],
              spacing: { after: 400 },
              shading: { fill: 'FEF3C7' },
            }),
            ...
            infoTable([
              ['Acheteur / Pouvoir adjudicateur :', form.acheteur || '—'],
            ]),
            br(),
    ...
    <div className="space-y-5">
        ...
        <div className="bg-surface border border-border rounded-2xl p-6 shadow-[0_2px_12px_rgba(37,99,235,0.04)]">
          ...
          <Field label="Sélectionner une analyse (optionnel)">
            <select
              ...
              onChange={e => {
                if (e.target.value) prefillFromAnalyse(e.target.value)
                else update('analyse_id', null)
              }}
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field label="Raison sociale">
              <input type="text" value={form.raison_sociale} onChange={e => update('raison_sociale', e.target.value)}
                placeholder="Entreprise SAS" className={inputClass} />
            </Field>
            <Field label="Forme juridique">
              <input type="text" value={form.forme_juridique} onChange={e => update('forme_juridique', e.target.value)}
                placeholder="SAS, SARL, SA…" className={inputClass} />
            </Field>
          </div>
          <Field label="Adresse du siège social">
            <input type="text" value={form.adresse_siege} onChange={e => update('adresse_siege', e.target.value)}
              placeholder="12 rue de la Paix, 75002 Paris" className={inputClass} />
          </Field>
          <Field label="N° SIRET">
            <input type="text" value={form.siret} onChange={e => update('siret', e.target.value)}
              placeholder="123 456 789 00012" className={inputClass} />
          </Field>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field label="Nom du signataire">
              <input type="text" value={form.nom_signataire} onChange={e => update('nom_signataire', e.target.value)}
                placeholder="Jean Dupont" className={inputClass} />
            </Field>
            <Field label="Qualité du signataire">
              <input type="text" value={form.qualite_signataire} onChange={e => update('qualite_signataire', e.target.value)}
                placeholder="Président, Gérant…" className={inputClass} />
            </Field>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Field label="IBAN (optionnel)">
              <input type="text" value={form.iban} onChange={e => update('iban', e.target.value)}
                placeholder="FR76 3000 6000…" className={inputClass} />
            </Field>
            <Field label="BIC / SWIFT (optionnel)">
              <input type="text" value={form.bic} onChange={e => update('bic', e.target.value)}
                placeholder="BNPAFRPPXXX" className={inputClass} />
            </Field>
          </div>
        </div>
      </div>
      ...
      <div className="bg-surface border border-border rounded-2xl p-6 shadow-[0_2px_12px_rgba(37,99,235,0.04)]">
        ...
        <div className="space-y-5">
          <Field label="Acheteur / Pouvoir adjudicateur">
            <input type="text" value={form.acheteur} onChange={e => update('acheteur', e.target.value)}
              placeholder="Commune de Paris, CHU de Bordeaux…" className={inputClass} />
          </Field>
          <Field label="Objet du marché">
            <textarea value={form.objet} onChange={e => update('objet', e.target.value)}
              placeholder="Travaux de rénovation thermique de l'école primaire…"
              rows={2} className={`${inputClass} resize-none`} />
          </Field>
          <Field label="Délai d'exécution">
            <input type="text" value={form.delai_execution} onChange={e => update('delai_execution', e.target.value)}
              placeholder="12 mois à compter de l'ordre de service" className={inputClass} />
          </Field>
        </div>
      </div>
      ...
      <div className="bg-surface border border-border rounded-2xl p-6 shadow-[0_2px_12px_rgba(37,99,235,0.04)]">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1">
                <input
                  ...
                  value={form.taux_tva}
                  onChange={e => update('taux_tva', e.target.value)}
            ...
            <div key={i} className="bg-background border border-border rounded-xl p-4 space-y-3">
              <div className="flex items-center justify-between gap-2">
                ...
                {form.lots.length > 1 && (
                  <button type="button" onClick={() => removeLot(i)}
                    className="p-1.5 text-text-subtle hover:text-red-500 rounded-lg hover:bg-red-50 transition-colors duration-150"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="space-y-1">
                  ...
                  <input type="text" value={lot.numero} onChange={e => updateLot(i, 'numero', e.target.value)}
                ...
                <div className="sm:col-span-2 space-y-1">
                  ...
                  <input type="text" value={lot.designation} onChange={e => updateLot(i, 'designation', e.target.value)}
                    placeholder="Gros œuvre, charpente…" className={inputClass} />
                </div>
              </div>
              <div className="space-y-1">
                <label className="font-syne text-[11px] font-semibold text-text-subtle">Montant HT (€)</label>
                <input type="number" min="0" step="0.01"
                  value={lot.montant_ht}
                  onChange={e => updateLot(i, 'montant_ht', e.target.value)}
                  placeholder="50000.00" className={inputClass} />
              </div>
        ...
        <button
          type="button"
          onClick={addLot}
          className="mt-4 flex items-center gap-1.5 font-syne text-[12px] font-semibold text-accent hover:text-accent-dark transition-colors duration-150"
        >
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div className="bg-surface border border-border rounded-2xl p-6 shadow-[0_2px_12px_rgba(37,99,235,0.04)]">
          ...
          <div className="space-y-3">
            {lotsCalcules.filter(l => l.ht > 0).map(l => (
              <CopyBlock
                key={l.numero}
                label={`Lot ${l.numero}${l.designation ? ' – ' + l.designation : ''}`}
                value={`${formatEur(l.ht)} € HT — ${montantEnLettres(l.ht)} hors taxes`}
              />
            ))}
            {form.lots.length > 1 && (
              <>
                <CopyBlock
                  ...
                  value={`${formatEur(totalHt)} € HT — ${montantEnLettres(totalHt)} hors taxes`}
                />
                <CopyBlock
                  label="Total global TTC"
                  value={`${formatEur(totalTtc)} € TTC — ${montantEnLettres(totalTtc)} toutes taxes comprises`}
                />
              </>
            )}
          </div>
      ...
      <div className="flex items-center justify-between gap-4 pt-1 flex-wrap">
        <div className="flex items-center gap-4">
          {saveState && 'success' in saveState && (
            <span className="font-syne text-[13px] font-semibold text-accent">Brouillon enregistré ✓</span>
          )}
          {saveState && 'error' in saveState && (
            <span className="font-syne text-[13px] font-semibold text-red-600">{saveState.error}</span>
          )}
        </div>
        <div className="flex items-center gap-3">
          <button
